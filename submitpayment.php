...
$razorpay_live_key= 'rzp_live_7XyxjwAowekmHy';
$razorpay_live_secret_key='cPhxEeoPO956n5SRtiWMO755';
...
// $order="INSERT INTO `orders` (`username`, `name`, `address`, `pincode`, `phone`, `email`, `order_date`, `status`, `amount`, `order_id`, `additional`) VALUES ('$username', '$billing_name', '$address', '$pincode', '$billing_mobile', '$billing_email', current_timestamp(), 'pending', '$payAmount', '$order_id', '$additional');";
// $result_order=mysqli_query($con,$order);
$order_query = "INSERT INTO orders (username, name, address, pincode, phone, email, order_date, status, amount, order_id, additional) VALUES (?, ?, ?, ?, ?, ?, current_timestamp(), 'pending', ?, ?, ?)";
$stmt = $con->prepare($order_query);
$stmt->bind_param("sssssdsss", $username, $billing_name, $address, $pincode, $billing_mobile, $billing_email, $payAmount, $order_id, $additional);
$result_order = $stmt->execute();
...
// $update="UPDATE `orders` SET `status` = 'success' WHERE `orders`.`order_id` = '$order_id';";
